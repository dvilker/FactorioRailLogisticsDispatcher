...
function getCountUnits()
    if countUnits0 == nil then
        ...
        countUnits0[1] = {
            ...
            item = true,
            fluid = true,
            signalCode = { type = "virtual", name = "signal-A" },
            calc = function(count)
                return count
            end,
        ...
        countUnits0[10] = {
            ...
            item = true,
            signalCode = { type = "virtual", name = "signal-B" },
        ...
        if game.entity_prototypes["cargo-wagon"] and game.entity_prototypes["cargo-wagon"].type == "cargo-wagon" then
            ...
            countUnits0[100] = {
                ...
                item = true,
                float = true,
                signalCode = { type = "item", name = "cargo-wagon" },
                calc = function(count, name)
            ...
            for _, proto in pairs(game.entity_prototypes) do
                if proto.type == "cargo-wagon" then
                    ...
                    countUnits0[100] = {
                        ...
                        item = true,
                        float = true,
                        signalCode = { type = "item", name = proto.name },
                        calc = function(count, name)
                            return math.floor(count * wagonStackCount * game.item_prototypes[name].stack_size)
                        end,
                        toString = function(count)
        ...
        if game.entity_prototypes["fluid-wagon"] and game.entity_prototypes["fluid-wagon"].type == "fluid-wagon" then
            ...
            countUnits0[200] = {
                ...
                fluid = true,
                float = true,
                signalCode = { type = "item", name = "fluid-wagon" },
                calc = function(count, name)
...
---@return UnitCode|nil
function getUnitBySignal(signal)
    local units = getCountUnits()
    for code, unit in pairs(units) do
        if unit.signalCode.name == signal.name and unit.signalCode.type == signal.type then
            return code
        end
    end
    return nil
