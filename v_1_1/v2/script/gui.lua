---@alias GuiName string|number @ names of elements
---@alias GuiUid string @ just unique string for each model
---@alias GuiKey string @ key for guiIndex
---@alias GuiRowId Any

---@shape GuiSelf

---@shape GuiRowModelInfo
    ---@field updateIndex number
    ---@field rows table<GuiRowId, GuiModel>
    ---@field rowDefs GuiDef[]

---@shape GuiHandlerModel
    ---@field uid GuiUid
    ---@field self GuiSelf

------@shape GuiModel: GuiHandlerModel
...
---@shape GuiDefHandlers
    ---@field onAny fun(self: GuiSelf)|fun(self: GuiSelf, event:EventData)|fun(self: GuiSelf, event:EventData, model:GuiModel)|nil
    ---@field onCheckedStateChanged fun(self: GuiSelf)|fun(self: GuiSelf, event:OnGuiCheckedStateChanged)|fun(self: GuiSelf, event:OnGuiCheckedStateChanged, model:GuiModel)|nil
    ---@field onClick fun(self: GuiSelf)|fun(self: GuiSelf, event:OnGuiClick)|fun(self: GuiSelf, event:OnGuiClick, model:GuiModel)|nil
    ---@field onElemChanged fun(self: GuiSelf)|fun(self: GuiSelf, event:OnGuiElemChanged)|fun(self: GuiSelf, event:OnGuiElemChanged, model:GuiModel)|nil
    ---@field onLocationChanged fun(self: GuiSelf)|fun(self: GuiSelf, event:OnGuiLocationChanged)|fun(self: GuiSelf, event:OnGuiLocationChanged, model:GuiModel)|nil
    ---@field onSelectedTabChanged fun(self: GuiSelf)|fun(self: GuiSelf, event:OnGuiSelectedTabChanged)|fun(self: GuiSelf, event:OnGuiSelectedTabChanged, model:GuiModel)|nil
    ---@field onSelectionStateChanged fun(self: GuiSelf)|fun(self: GuiSelf, event:OnGuiSelectionStateChanged)|fun(self: GuiSelf, event:OnGuiSelectionStateChanged, model:GuiModel)|nil
    ---@field onSwitchStateChanged fun(self: GuiSelf)|fun(self: GuiSelf, event:OnGuiSwitchStateChanged)|fun(self: GuiSelf, event:OnGuiSwitchStateChanged, model:GuiModel)|nil
    ---@field onTextChanged fun(self: GuiSelf)|fun(self: GuiSelf, event:OnGuiTextChanged)|fun(self: GuiSelf, event:OnGuiTextChanged, model:GuiModel)|nil
    ---@field onValueChanged fun(self: GuiSelf)|fun(self: GuiSelf, event:OnGuiValueChanged)|fun(self: GuiSelf, event:OnGuiValueChanged, model:GuiModel)|nil
    ---@field onClosed fun(self: GuiSelf)|fun(self: GuiSelf, event:OnGuiClosed)|fun(self: GuiSelf, event:OnGuiClosed, model:GuiModel)|nil
...
local definesEventsAny = --[[---@type defines__events]]-1000
local definesEventsAnyString = "-1000"

---@type table<defines__events, string>
local eventMap = {
...
local functionNameCache = {}

---@param obj table
---@param func function
local function getFunctionName(obj, func)
    local cachedName = functionNameCache[func]
    if cachedName then
        if obj[cachedName] == func then
            return cachedName
        else
            error("Function " .. var_dump_light(cachedName) .. " renamed or redefined or wrong used in " .. var_dump_light(obj))
        end
    end
    for name, val in pairs(obj) do
        if val == func then
            functionNameCache[func] = name
            return name
        end
    end
    ...
    error("Function " .. var_dump_light(func) .. " is not found in " .. var_dump_light(obj))
end
...
local function addElement(target, def, model)
    ...
    for defineEvent, prop in pairs(eventMap) do
        if def[prop] then
            ...
            local functionName = getFunctionName(model.self, func)
            tags = tags or {}
            tags._model = model.uid
            tags._modelEvents = tags._modelEvents or {}
            tags._modelEvents[guiEventsToString[defineEvent]] = functionName
            def[prop] = nil
        end
    end
    ...
    if defSub then
        if def._autoSharesFrom and def.column_count then
            for i, sub in pairs(defSub) do
                ...
                local fromElement = (--[[---@not nil]]defSub)[j]
...
function createGuiModel(target, self, rootDef)
    ...
    ---@type GuiModel
    local model = {
        uid = --[[---@type GuiUid]] guiModelUniquePrefix .. tostring(uid),
        self = self,
        named = {},
        shares = {},
        tables = {},
...
function destroyGuiModel(model)
    ...
    if model._key then
        if storage.guiIndex then -- TODO remove
        ...
        for _, player in pairs(game.connected_players) do
            for _, rootGui in pairs(player.gui.children) do
                for _, someGui in pairs(rootGui.children) do
                    local modelUid = --[[---@type GuiUid]]someGui.tags._model
...
local function guiHandleEvent(event)
    ...
    if event.element.tags._modelEvents then
        local byEvent = event.element.tags._modelEvents[guiEventsToString[event.name]]
        local byAny = event.element.tags._modelEvents[definesEventsAnyString]
        -- [[DEBUG]]log("GUI byEvent " .. var_dump(byEvent) .. ", byAny: " .. var_dump(byAny))
        if byEvent or byAny then
            local modelUid = --[[---@type GuiUid]]event.element.tags._model
            -- [[DEBUG]]log("GUI modelUid " .. var_dump(modelUid))
            if modelUid then
                local model = storage.guiModels[modelUid]
                -- [[DEBUG]]log("GUI model " .. var_dump_light(model))
                if model then
                    if byEvent then
                        model.self[byEvent](model.self, event, model)
                    end
                    if byAny then
                        model.self[byAny](model.self, event, model)
                    end
                end
            end
        end
...
function setGuiHandlerTags(uid, handlers, tags)
    local self = storage.guiModels[uid].self
    for defineEvent, prop in pairs(eventMap) do
        if handlers[prop] then
            ...
            local functionName = getFunctionName(self, func)
            tags = tags or {}
            tags._model = uid
            tags._modelEvents = tags._modelEvents or {}
            tags._modelEvents[guiEventsToString[defineEvent]] = functionName
